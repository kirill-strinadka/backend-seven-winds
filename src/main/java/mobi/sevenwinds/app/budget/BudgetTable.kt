...
object BudgetTable : IntIdTable("budget") {
    ...
    val author = reference("author_id", AuthorTable).nullable() // Добавление внешнего ключа на AuthorTable
}
...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toResponseWithAuthor(): BudgetRecordResponse {
        return BudgetRecordResponse(year, month, amount, type, author?.fullName, author?.createdAt.toString())
    }
}